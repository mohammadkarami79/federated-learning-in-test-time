...
# -*- coding: utf-8 -*-
"""
Kim et al., 2023 Reproduction - FINAL FIX
=========================================

This script uses the EXACT architecture from the existing codebase
to ensure compatibility with trained checkpoints.
"""
...
class Kim2023Config:
    ...
    # Diffusion settings (match existing)
...
class L2PGDAttack:
    """L2-norm PGD attack for Kim et al., 2023 reproduction - FIXED VERSION"""
    ...
    def attack(self, model, images, labels):
        """Generate L2-PGD adversarial examples with proper gradient handling"""
        # Set model to eval but don't use no_grad context
        model.eval()
        ...
        # PGD iterations (OUTSIDE no_grad context)
        for step in range(self.steps):
            # Create a fresh tensor that requires grad
...
def load_defense_components(config: Kim2023Config, logger: logging.Logger):
    """Load pre-trained defense components using EXACT existing architectures"""
    components = {}
    ...
    # Load DiffPure model using EXACT architecture from diffusion/diffuser.py
    if config.USE_DIFFPURE:
        try:
            # Import the EXACT UNet class from the existing codebase
            from diffusion.diffuser import UNet
            
            # Use the same parameters as integrated_diffusion_training.py
            diffuser = UNet(
                in_channels=config.IMG_CHANNELS,
                hidden_channels=config.DIFFUSION_HIDDEN_CHANNELS,  # 128
            ...
            if diffusion_checkpoint.exists():
                try:
                    ...
                    logger.info(f"SUCCESS: Loaded diffusion model: {diffusion_checkpoint}")
...
def diffpure_purify(diffuser, adv_data, config):
    ...
    diffuser.eval()
    with torch.no_grad():
        # Simple denoising (using existing forward method)
        # The exact forward method depends on the UNet implementation
        try:
            # Try with time parameter (if required)
            batch_size = adv_data.size(0)
            t = torch.zeros(batch_size, device=adv_data.device)
            purified = diffuser(adv_data, t)
            return torch.clamp(purified, 0, 1)
        except:
            # Try without time parameter
            try:
                purified = diffuser(adv_data)
                return torch.clamp(purified, 0, 1)
            except:
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='./kim2023_results_final', help='Output directory')
    ...
    logger.info("============================================================")
    logger.info("KIM ET AL., 2023 REPRODUCTION EXPERIMENT - FINAL FIX")
    logger.info("============================================================")
    ...
    logger.info(f"Formula: x_next = Pi_x+S(x_t + alpha * grad_x L(h_theta, x, y))")
    logger.info("========================================")
