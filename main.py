...
def run_federated(cfg):
    ...
    for round_idx in range(cfg.N_ROUNDS):
        ...
        server.receive_update(round_idx, client_models)
        global_models = server.aggregate()
        
        # Distribute aggregated models back to clients
        for client_idx, client in enumerate(clients):
            client.model.load_state_dict(global_models[client_idx].state_dict())
