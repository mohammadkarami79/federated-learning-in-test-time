...
class Adv_MixtureClient(MixtureClient):
    ...
    def generate_adversarial_data(self):
        ...
        # Use the device specified in the model instead of hardcoding cuda
        device = next(self.learners_ensemble.learners[0].model.parameters()).device
        self.adv_nn.pgd_sub(self.atk_params, x_data.to(device), y_data.to(device))
...
class Adv_MixtureClient_DVERGE(MixtureClient):
    ...
    def generate_adversarial_data(self):
        ...
        for i in range(num_nns):
            ...
            # Use the device specified in the model instead of hardcoding cuda
            device = next(self.learners_ensemble.learners[0].model.parameters()).device
            self.adv_nns[i].pgd_sub(self.atk_params, x_data.to(device), y_data.to(device))
