...
class Server:
    ...
    def init_models(self, client_model: nn.Module):
        ...
        if self.global_models is None:
            self.global_models = self._clone_model(client_model)
        
    def receive_update(self, client_id: int, local_model: nn.Module):
        """Receive and store client updates for later aggregation."""
        if self.global_models is None:
            self.init_models(local_model)
        self.client_updates[client_id] = local_model
        
    def aggregate(self) -> nn.Module:
        """Perform federated aggregation - average model parameters."""
        n_clients = len(self.client_updates)
        assert n_clients > 0, "No updates received"
        
        # Average the model parameters
        avg_state = self.global_models.state_dict()
        
        # For each parameter
        for key in avg_state:
            # For BatchNorm layers, we don't average statistics
            if 'running_mean' in key or 'running_var' in key or 'num_batches_tracked' in key:
                continue
                
            # Stack parameters from all clients and average
            avg_state[key] = torch.stack([
                client_model.state_dict()[key] 
                for client_model in self.client_updates.values()
            ]).mean(dim=0)
            
        # Load averaged weights back
        self.global_models.load_state_dict(avg_state)
            
        # Clear updates and return new global model
        self.client_updates.clear()
        return self.global_models
