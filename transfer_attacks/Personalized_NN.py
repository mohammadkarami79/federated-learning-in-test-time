...
class Personalized_NN(nn.Module):
    ...
    def __init__(self, trained_network):
        ...
        # Get device from the trained network instead of hardcoding
        self.device = next(trained_network.parameters()).device
...
class Adv_NN(Personalized_NN):
    ...
    def pgd_sub(self, atk_params, x_in, y_in, x_base = None):
        ...
        for i in range(iteration):
            ...
            x_val_min = x_val_min.to(self.x_adv.device)
            x_val_max = x_val_max.to(self.x_adv.device)
            self.x_adv = torch.clamp(self.x_adv, x_val_min, x_val_max)
            self.x_adv = Variable(self.x_adv.data, requires_grad=True)
    ...
    def pgd(self, atk_params, print_info=False, mode='test'):
        ...
        for i in range(iteration):
            ...
            self.x_adv = torch.clamp(
                self.x_adv, 
                torch.tensor(x_val_min, device=self.x_adv.device), 
                torch.tensor(x_val_max, device=self.x_adv.device)
            )
            self.x_adv = Variable(self.x_adv.data, requires_grad=True)
