"""
BR35H Inverted Defense Configuration
MAE Detection + Inverted DiffPure (apply to clean, skip adversarial)
"""
...
def get_config():
    ...
    config = {
        ...
        # Inverted DiffPure settings - apply to clean samples only
        'ENABLE_DIFFPURE': True,
        'DIFFUSER_STEPS': 2,
        'DIFFUSER_SIGMA': 0.1,
        'DIFFUSER_SCHEDULE': 'linear',
        'DIFFPURE_STRENGTH': 0.2,
        'DIFFPURE_ADAPTIVE': True,
        'INVERTED_DEFENSE': True,  # NEW: Apply DiffPure to clean samples only
        ...
        'HYBRID_DEFENSE_MODE': 'inverted',  # Inverted hybrid defense
...
if __name__ == "__main__":
    cfg = get_config()
    print("BR35H Inverted Defense Configuration:")
    print(f"MAE_THRESHOLD: {cfg['MAE_THRESHOLD']} (working)")
    print(f"INVERTED_DEFENSE: {cfg['INVERTED_DEFENSE']} (NEW)")
    ...
    print("Expected: High adversarial accuracy by skipping DiffPure for detected samples")
