"""
BR35H Ultra Mild Defense Configuration
...
def get_config():
    """Get ultra mild defense configuration"""
    config = {
        ...
        # Ultra Mild DiffPure settings - very gentle
        'ENABLE_DIFFPURE': True,
        'DIFFUSER_STEPS': 1,  # Only 1 step
        'DIFFUSER_SIGMA': 0.05,  # Very low noise
        'DIFFUSER_SCHEDULE': 'linear',
        'DIFFPURE_STRENGTH': 0.05,  # Very gentle strength (5%)
        'DIFFPURE_ADAPTIVE': True,
        ...
        'HYBRID_DEFENSE_MODE': 'ultra_mild',  # Ultra mild defense
        ...
        'TARGET_ADV_ACC': 85.0,  # Target: 85-90%
...
if __name__ == "__main__":
    cfg = get_config()
    print("BR35H Ultra Mild Defense Configuration:")
    print(f"MAE_THRESHOLD: {cfg['MAE_THRESHOLD']} (working)")
    print(f"DIFFUSER_STEPS: {cfg['DIFFUSER_STEPS']} (only 1 step)")
    print(f"DIFFUSER_SIGMA: {cfg['DIFFUSER_SIGMA']} (very low)")
    print(f"DIFFPURE_STRENGTH: {cfg['DIFFPURE_STRENGTH']} (very gentle - 5%)")
    print(f"TARGET_ADV_ACC: {cfg['TARGET_ADV_ACC']}% (85-90%)")
    print("Expected: High adversarial accuracy with very mild purification")
